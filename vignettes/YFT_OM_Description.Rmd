...
author: |  
  | __YELLOWFIN TUNA__ 
  | __OPERATING MODEL DESCRIPTION__
...
Recent YFT stock assessments performed for the Indian Ocean Tuna Commission (IOTC), as well as supplementary papers that describe the operating model (i.e., the simulation model) in more detail are also provided on the Spatial Stock Assessment Workshop [GitHub repository](https://github.com/carenbarcelo-NOAA/Spatial-Assessment-Modeling-Workshop/tree/main/Background%20files%20for%20participants). While the recent operational YFT stock assessment is provided as general background (Fu et al., (2018), Cardinale et al. (2020)), this simulation experiment is strictly a research endeavor, with the Indian Ocean YFT stock chosen as an example to address broad study objectives. Analysts should be aware that the reality simulated by the YFT OM differs substantially in some respects from the real IO-YFT population. 

The Indian Ocean YFT operating model developed for this simulation experiment is described below.  It is based on a previous version described in detail in [Dunn et al. (2020)](https://github.com/carenbarcelo-NOAA/Spatial-Assessment-Modeling-Workshop/blob/main/Background%20files%20for%20participants/IOTC%20YFT%20OM%20application/Dunn%2C%20Hoyle%2C%20Datta%20(2020)%20IOTC-2020-YFT.pdf), but with key differences. Specifically, it incorporates time-varying spatial recruitment apportionment layers, a longer time series of stochastic recruitment deviations, and fixes an issue with the previously used growth curve. 

# 2.	Architecture of the YFT OM and prior model conditioning
Using the Spatial Population Model software (SPM, developed by A. Dunn, S. Rasmussen, and S. Mormede, available [here](https://github.com/alistairdunn1/SPM)), a spatially explicit population dynamics OM was developed for YFT, a highly migratory pelagic species, in the Indian Ocean. This model was conditioned using empirical data from region-specific fisheries and spatially explicit (disaggregated) catch, longline CPUE, tag recapture data, and length frequencies at a 5x5$^\circ$ grid cell resolution (221 total cells). The OM was conditioned by fitting the disaggregated IOTC data within SPM itself using maximum likelihood to estimate fishery and population parameter values (e.g., fishery selectivity, fishing mortality, recruitment, and movement; see [Dunn et al. (2020)](https://github.com/carenbarcelo-NOAA/Spatial-Assessment-Modeling-Workshop/blob/main/Background%20files%20for%20participants/IOTC%20YFT%20OM%20application/Dunn%2C%20Hoyle%2C%20Datta%20(2020)%20IOTC-2020-YFT.pdf) for further details). R~0~, recruitment deviations and recruitment apportionment were fixed based on values described in a recent stock assessment conducted by Fu et al. (2018). The conditioned YFT SPM model was then used as an operating model to simulate pseudo-data with observation error, such that these *simulated observations* provide plausible values for Indian Ocean YFT. Each 5x5$^\circ$ grid cell within the spatial domain of the SPM OM is treated as a unique population sub-unit with local recruitment, mortality, and movement dynamics.  
...
The previous version of the YFT OM ([Dunn et al. (2020)](https://github.com/carenbarcelo-NOAA/Spatial-Assessment-Modeling-Workshop/blob/main/Background%20files%20for%20participants/IOTC%20YFT%20OM%20application/Dunn%2C%20Hoyle%2C%20Datta%20(2020)%20IOTC-2020-YFT.pdf)) did not include stochasticity in recruitment. We introduced stochasticity into the YFT OM used in this simulation experiment by introducing process error in recruitment across space (spatial apportionment) and time (year class strength; YCS). Thus, simulated observations were generated for 100 different realizations of the OM, integrating both process and observation sources of error. 
...
knitr::include_graphics(rep('C:/Users/caren.barcelo/Documents/images/baseplot_gridwlabels_5regions.png', 1))
```
...
$$
L_{i,a} = L_{inf,a} + (L_{i-1,a} - L_{inf,a}) e^{-k_{a,i-1}} \tag{Eq. 1}
$$
...
```{r knitr-logo3, out.width='75%',fig.align="center"}
knitr::include_graphics(rep('C:/Users/caren.barcelo/Documents/images/biologicalparameters_v2.png', 1))
```
...
```{r knitr-logo2, out.width='75%',fig.align="center"}
knitr::include_graphics(rep('C:/Users/caren.barcelo/Documents/images/Dunn_recruitment.png', 1))
```
...
```{r knitr-logo4, out.width='75%',fig.align="center"}
knitr::include_graphics(rep('C:/Users/caren.barcelo/Documents/images/totalcatch_barplotbyfleet.png', 1))
```
...
```{r knitr-logo5, out.width='75%',fig.align="center"}
knitr::include_graphics(rep('C:/Users/caren.barcelo/Documents/images/totalcatch_N_mt.png', 1))
```
...
```{r knitr-logo6, out.width='100%',fig.align="center"}
knitr::include_graphics(rep('C:/Users/caren.barcelo/Documents/images/diff_fisheries_catch_spatial.png', 1))
```
...
```{r knitr-logo7, out.width='65%',fig.align="center"}
knitr::include_graphics(rep('C:/Users/caren.barcelo/Documents/images/tagreleaserecapt_movement.png', 1))
```
...
```{r knitr-logo9, out.width='75%',fig.align="center"}
knitr::include_graphics(rep('C:/Users/caren.barcelo/Documents/images/data_plot.png',1))
```
...
```{r knitr-logo10, out.width='75%',fig.align="center"}
knitr::include_graphics(rep('C:/Users/caren.barcelo/Documents/images/data_plot_1a.png',1))
```
...
|:---------------------------:|:-------------:|:-------------:|
| YFT_1area_observations_1-100.RData | Single area (panmictic population) YFT data | [here](https://github.com/carenbarcelo-NOAA/Spatial-Assessment-Modeling-Workshop/tree/main/YFT_OM_DATASETS/1area) |
| YFT_4area_observations_1-100.RData | Four area (four area aggregation) YFT data | [here](https://github.com/carenbarcelo-NOAA/Spatial-Assessment-Modeling-Workshop/tree/main/YFT_OM_DATASETS/4area) |
| YFT_221cell_observations_1-100.RData | Fully spatial (5x5 binned) YFT data | [here](https://github.com/carenbarcelo-NOAA/Spatial-Assessment-Modeling-Workshop/tree/main/YFT_OM_DATASETS/221cell) |
